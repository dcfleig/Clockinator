...
void display_Callback(const char *pJsonString, uint32_t JsonStringDataLen, jsonStruct_t *pContext)
{
    IOT_UNUSED(pJsonString);
    IOT_UNUSED(JsonStringDataLen);
...
void aws_iot_task(void *param)
{
    ...
    if (SUCCESS != rc)
    {
        ESP_LOGE(TAG, "Shadow Register SNTP Delta Error");
    }

    rc = aws_iot_shadow_register_delta(&mqttClient, &displayHandler);
    if (SUCCESS != rc)
    {
